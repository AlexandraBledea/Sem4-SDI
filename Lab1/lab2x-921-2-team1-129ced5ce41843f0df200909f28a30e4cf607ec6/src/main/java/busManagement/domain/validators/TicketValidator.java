...
public class TicketValidator implements Validator<Ticket>{
    ...
    public void validate(Ticket entity) throws ValidatorException {
        ...
        final Pair<Boolean, String> resultPassagerId =
                new Pair<>(passagerIdIsNullOrNegative.test(entity.getPassengerId()), "PassagerID must not be null or negative!\n");
        ...
        Predicate<Long> boardingTimeNullOrInvalid = (i) -> (i == null || i < 0 || i > 23);
        final Pair<Boolean, String> resultBoardingTime =
                new Pair<>(boardingTimeNullOrInvalid.test(entity.getBoardingTime()), "Ticket must have a non-null BoardingTime and it must be in the (0, 23) range!\n");

        Predicate<Long> priceNullOrNegative = (i) -> (i == null || i < 0);
        final Pair<Boolean, String> resultPrice =
                new Pair<>(priceNullOrNegative.test(entity.getBoardingTime()), "Ticket must have a non-null and not negative Price!\n");
