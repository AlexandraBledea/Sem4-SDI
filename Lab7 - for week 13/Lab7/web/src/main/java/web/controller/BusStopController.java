...
//        return busStopConverter.convertModelsToDTOs(stops);
    ...
    @RequestMapping(value = "/stops/{stopTime}/{pageNumber}/{pageSize}")
    ResponseEntity<PagingResponse<BusStopDTO>> findAll(@PathVariable String stopTime, @PathVariable Integer pageNumber, @PathVariable Integer pageSize){
        PagingRequest pagingRequest = new PagingRequest(pageNumber, pageSize);
        PagingResponse<BusStopDTO> stops = busStopService.findAllFilteredByStopTime(pagingRequest, stopTime);
        return new ResponseEntity<>(stops, HttpStatus.OK);
    }

    @RequestMapping(value = "/stops/{pageNumber}/{pageSize}")
    ResponseEntity<PagingResponse<BusStopDTO>> findAll(@PathVariable Integer pageNumber, @PathVariable Integer pageSize){
        PagingRequest pagingRequest = new PagingRequest(pageNumber, pageSize);
        PagingResponse<BusStopDTO> stops = busStopService.findAll(pagingRequest);
        return new ResponseEntity<>(stops, HttpStatus.OK);
    }
