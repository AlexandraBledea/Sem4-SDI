...
import {BusStationsDTO} from "../../shared/stations.model";
...
export class CitiesService {
  ...
  findAllStationsForCity(id: number): Observable<BusStationsDTO>{
    let url = `${this.backendUrl}/${id}`
    return this.httpClient.get<BusStationsDTO>(url);
  }

  findAllStopsForStationAndCity(cityId: number, stationId: number): Observable<BusStopsDTO>{
    let url = `${this.backendUrl}/${cityId}/${stationId}`
    return this.httpClient.get<BusStopsDTO>(url);
  }
